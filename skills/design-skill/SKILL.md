...
| 步骤 | 状态文件步骤 ID |
|------|----------------|
| 步骤一：理解需求 | `design-step-1` |
| 步骤二：定义信息架构 | `design-step-2` |
| 步骤三：梳理用户流程 | `design-step-3` |
| 步骤四：定义设计令牌 | `design-step-4` |
| 步骤五：规格化组件 | `design-step-5` |
| 步骤六：设计页面/屏幕 | `design-step-6` |
| 步骤七：定义交互模式 | `design-step-7` |
| 步骤八：验证无障碍 | `design-step-8` |

### 更新规则

通过 `notify-state.mjs` 辅助脚本更新状态（Dashboard 运行时走 API 即时广播，未运行时 fallback 到原子文件写入）。本阶段（phase-id = 2）的步骤开始/完成命令、活动日志追加、`--result` 必填等通用约定见 [workflow-skill/references/sub-skill-state-updates.md](../workflow-skill/references/sub-skill-state-updates.md)。

> **步骤八产出制品**：设计规范、用户流程、页面规格、设计稿 HTML、交互说明 HTML 等会自动注册到 `phases[2].artifacts`。
