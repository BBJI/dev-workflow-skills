...
2. 找到 `phases[6].steps` 中对应步骤 ID 的条目
3. 设置 `status` 为 "in-progress"，`startedAt` 为当前 ISO 时间戳
4. 在 `activityLog` 末尾追加：`{ timestamp, phase: 6, action: "step-started", message: "{步骤名}", level: "info" }`
5. 更新 `updatedAt`，写回文件

**步骤完成时**：
1. 读取 `.dws/{项目名}/workflow-state.json`
2. 找到对应步骤，设置 `status` 为 "completed"，`completedAt` 为当前 ISO 时间戳
3. 将本步骤产出的文件添加到 `phases[6].artifacts` 数组（任务完成报告）
4. 在 `activityLog` 末尾追加：`{ timestamp, phase: 6, action: "step-completed", message: "{步骤名}", level: "success" }`
5. 更新 `updatedAt`，写回文件

**步骤三（TDD实现）子步骤更新**：可在 `detail` 字段中写入当前子步骤（"Red: 编写失败测试"、"Green: 最小实现"、"Refactor: 重构"），帮助仪表盘展示更细粒度的进度。

**步骤六（Bug修复）完成时**：追加活动日志：`{ timestamp, phase: 6, action: "bug-fixed", message: "修复Bug: {Bug描述摘要}", level: "success" }`
