...
node "$SKILL_DIR/dashboard/notify-state.mjs" --project-root "$PROJECT_ROOT" --project-name "$PROJECT_NAME" \
  --type step --phase-id 6 --step-id {步骤ID} --status in-progress --detail "简要描述"
```

**步骤完成时**：
```bash
node "$SKILL_DIR/dashboard/notify-state.mjs" --project-root "$PROJECT_ROOT" --project-name "$PROJECT_NAME" \
  --type step --phase-id 6 --step-id {步骤ID} --status completed --result "步骤执行结果摘要"
```

**追加活动日志**（步骤开始/完成时可选附加）：
```bash
node "$SKILL_DIR/dashboard/notify-state.mjs" --project-root "$PROJECT_ROOT" --project-name "$PROJECT_NAME" \
  --type activity --phase 6 --action step-started --message "{步骤名}" --level info
```
